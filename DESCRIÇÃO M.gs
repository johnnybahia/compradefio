function BUSCAR_PEDIDO_M(intervalo) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const assoc = ss.getSheetByName("ASSOCIAÇÃO");
  const pedido = ss.getSheetByName("PEDIDO DE FIO");

  const assocData = assoc.getRange("A2:G" + assoc.getLastRow()).getValues();
  const pedidoData = pedido.getRange("M7:O" + pedido.getLastRow()).getValues();

  const mapaPedido = {};
  pedidoData.forEach(row => {
    if (String(row[2]).trim() !== "")
      mapaPedido[String(row[2]).trim().toUpperCase()] = String(row[0]).trim();
  });
