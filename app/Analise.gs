...
var RELACAO_COMPRA_HEADERS = [
  ...
  'SALDO',         // saldo no fim do período analisado
  'CONSUMO_PERIODO', // consumo dentro do período informado
...
function gerarRelacaoDeCompra(token, params) {
  ...
  var dataInicio = _parseDataISO(params.dataInicio);
  var dataFim = _parseDataISO(params.dataFim);
  ...
  // Período de análise: [dataInicio, dataFim].
  //
  // var saldos   = _lerSaldosNaData(dataFim);              // estoque no fim do período
  // var baixos   = _filtrarEstoqueBaixo(saldos);
  // var consumo  = _consumoNoPeriodo(baixos, dataInicio, dataFim);
